...
C     
C     Process: u u~ > ta- ta+ NP<=2 / zp gp @1
C     Process: c c~ > ta- ta+ NP<=2 / zp gp @1
C     
      USE DISCRETESAMPLER
      IMPLICIT NONE
C     
...
C     Amplitude(s) for diagram number 1
      ...
      CALL FFV2_3_3(W(1,1),W(1,2),GC_118,GC_107,MDL_MZ, FK_MDL_WZ,W(1
     $ ,5))
...
C          ENDDO
C          DO N = 1, NAMPSO
C            IF (CHOSEN_SO_CONFIGS(SQSOINDEX4(M,N))) THEN
C              MATRIX4 = MATRIX4 + ZTEMP*DCONJG(JAMP(I,N))/DENOM(I)
C            ENDIF
C          ENDDO
C        ENDDO
C      ENDDO
