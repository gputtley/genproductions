...
C     
C     Process: s d~ > ta- ta+ NP<=2 / zp gp @1
C     
      USE DISCRETESAMPLER
      IMPLICIT NONE
C     
...
C     Amplitude(s) for diagram number 1
      CALL FFV2_0(W(1,4),W(1,2),W(1,5),GC_21,AMP(1))
...
C          ENDDO
C          DO N = 1, NAMPSO
C            IF (CHOSEN_SO_CONFIGS(SQSOINDEX7(M,N))) THEN
C              MATRIX7 = MATRIX7 + ZTEMP*DCONJG(JAMP(I,N))/DENOM(I)
C            ENDIF
C          ENDDO
C        ENDDO
C      ENDDO
