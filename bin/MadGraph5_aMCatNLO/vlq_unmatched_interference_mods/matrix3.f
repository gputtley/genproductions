...
C     
C     Process: b b~ > ta- ta+ NP<=2 / zp gp @1
C     
      USE DISCRETESAMPLER
      IMPLICIT NONE
C     
...
C     
      ...
      REAL*8 MATRIX_SM
      REAL*8 MATRIX_VLQ
      COMPLEX*16 AMP(NGRAPHS), JAMP(NCOLOR,NAMPSO), JAMP_SM(NCOLOR,NAMPSO), JAMP_VLQ(NCOLOR,NAMPSO)
...
C     ----------
     ...
     $   *SMALL_WIDTH_TREATMENT)), MDL_WZ)
        FK_MDL_WVLQ = SIGN(MAX(ABS(MDL_WVLQ), ABS(MDL_MVLQ
     $   *SMALL_WIDTH_TREATMENT)), MDL_WVLQ)
...
C     Amplitude(s) for diagram number 1
      ...
      CALL FFV2_3_3(W(1,1),W(1,2),GC_117,GC_106,MDL_MZ, FK_MDL_WZ,W(1
     $ ,5))
C     Amplitude(s) for diagram number 2
      CALL FFV2_3_0(W(1,4),W(1,3),W(1,5),GC_119,GC_109,AMP(2))
      CALL FFV2_3_3(W(1,1),W(1,3),GC_24,GC_25,MDL_MVLQ, FK_MDL_WVLQ
     $ ,W(1,5))
C     Amplitude(s) for diagram number 3
      CALL FFV2_3_0(W(1,4),W(1,2),W(1,5),GC_24,GC_25,AMP(3))
...
C get SM-only
      ...
      DO M = 1, NAMPSO
        DO I = 1, NCOLOR
          ZTEMP = (0.D0,0.D0)
          DO J = 1, NCOLOR
            ZTEMP = ZTEMP + CF(J,I)*JAMP_SM(J,M)
          ENDDO
          DO N = 1, NAMPSO
            IF (CHOSEN_SO_CONFIGS(SQSOINDEX3(M,N))) THEN
              MATRIX_SM = MATRIX_SM + ZTEMP*DCONJG(JAMP_SM(I,N))/DENOM(I)
            ENDIF
          ENDDO
        ENDDO
      ENDDO
...
C get VLQ-only
      ...
      DO M = 1, NAMPSO
        DO I = 1, NCOLOR
          ZTEMP = (0.D0,0.D0)
          DO J = 1, NCOLOR
            ZTEMP = ZTEMP + CF(J,I)*JAMP_VLQ(J,M)
          ENDDO
          DO N = 1, NAMPSO
            IF (CHOSEN_SO_CONFIGS(SQSOINDEX3(M,N))) THEN
              MATRIX_VLQ = MATRIX_VLQ + ZTEMP*DCONJG(JAMP_VLQ(I,N))/DENOM(I)
            ENDIF
          ENDDO
        ENDDO
      ENDDO

C subtract SM-only and VLQ-only to get only SM-VLQ interference


      MATRIX3 = MATRIX3 - MATRIX_SM - MATRIX_VLQ



C end of get SM-only
